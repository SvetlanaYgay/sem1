...
def delite_contact():
    delite = str(input("Введите имя или фамилию, или номер контакта, который хотите удалить: "))
    with open('phone_book.txt', 'r',encoding='utf-8') as data:
        text = data.read()
        ...
        for i, line in enumerate(text_lines):
            if delite in line:
                ...
                text_lines.pop(i)
                print(f'Запись удалена: {del_phone_book_lines}\n\n')
                break
...
def replace_line():
    rep_lace = str(input('Введите имя либо номер: '))
    with open('phone_book.txt', 'r',encoding='utf-8') as data:
        text = data.read()
        text_lines = text.strip().split('\n\n')
        for i, line in enumerate(text_lines):
            if rep_lace in line:
                ...
                print(f'изменить на: {rep_phone_book_lines}')
